		...
		// Calculate the number of columns, rows & total cells from the coordinates given
       	...
       	for(int i = 0; i < numcells-1; i++){
       		...
       		for(int j = numcells-1; j > i; j--){
       			...
       			// System.out.println("--- TEST (i=" + i + "|j=" + j + "): A[" + ((j/numrows)+sc) + "][" + ((j%numrows)+sr) + "] < A[" + (((j-1)/numrows)+sc) + "][" + (((j-1)%numrows)+sr) + "] | ?("
       				// + a[(j/numrows)+sc][(j%numrows)+sr].toString() + " < " + a[((j-1)/numrows)+sc][((j-1)%numrows)+sr].toString() + ")");

   				// Ignore all of these print lines, they're just to help you see the process in the console window
       			if (sort.equals("A")) {

       				System.out.print("--- TEST (i=" + i + "|j=" + j + "): A[" + ((j/numrows)+sc) + "][" + ((j%numrows)+sr) + "] < A[" + (((j-1)/numrows)+sc) + "][" + (((j-1)%numrows)+sr) + "] | ?("
           					+ a[(j/numrows)+sc][(j%numrows)+sr].toString() + " < " + a[((j-1)/numrows)+sc][((j-1)%numrows)+sr].toString() + ")");
           			
       				if (a[(j/numrows)+sc][(j%numrows)+sr].age < a[((j-1)/numrows)+sc][((j-1)%numrows)+sr].age) {

       					// Ignore the printing
       					System.out.println("  YES.");
       					System.out.println(">>> SWAP: A[" + ((j/numrows)+sc) + "][" + ((j%numrows)+sr) + "] < A[" + (((j-1)/numrows)+sc) + "][" + (((j-1)%numrows)+sr) + "]:");
                    
       					// Make the swap
       					temp = a[(j/numrows)+sc][(j%numrows)+sr];
       					a[(j/numrows)+sc][(j%numrows)+sr] = a[((j-1)/numrows)+sc][((j-1)%numrows)+sr];
       					a[((j-1)/numrows)+sc][((j-1)%numrows)+sr] = temp;
